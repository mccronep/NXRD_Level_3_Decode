...
# NAME:
# :::::::::::::::::::::::::::::::::::::::::::::::
# process_level3_NST_v7c_azran.py
# :::::::::::::::::::::::::::::::::::::::::::::::
#
#  PROGRAM OVERVIEW:
...
for item in storm_id:
    ...
    if (pos30_rng[ii] == 999) or (pos30_az[ii] == 999):
        pos30_rng[ii] = pos15_rng[ii] 
        pos30_az[ii]  = pos15_az[ii]
    if (pos45_rng[ii] == 999) or (pos45_az[ii] == 999):
        pos45_rng[ii] = pos30_rng[ii] 
        pos45_az[ii]  = pos30_az[ii]
    if (pos60_rng[ii] == 999) or (pos60_az[ii] == 999):
        pos60_rng[ii] = pos45_rng[ii] 
        pos60_az[ii]  = pos45_az[ii]

    #azlist=[angl, float(pos15_az[ii])*degtorad, float(pos30_az[ii])*degtorad, float(pos45_az[ii])*degtorad]
    azlist=[angl, float(pos15_az[ii])*degtorad, float(pos30_az[ii])*degtorad, float(pos45_az[ii])*degtorad, float(pos60_az[ii])*degtorad]
    #rangelist=[rng, float(pos15_rng[ii])*1.852, float(pos30_rng[ii])*1.852, float(pos45_rng[ii])*1.852]
    rangelist=[rng, float(pos15_rng[ii])*1.852, float(pos30_rng[ii])*1.852, float(pos45_rng[ii])*1.852, float(pos60_rng[ii])*1.852]

    ax.plot(azlist, rangelist, 'o-', color='black')
    #
    ax.plot(float(pos15_az[ii])*degtorad,float(pos15_rng[ii])*1.852, marker='o', color='green')
    ax.plot(float(pos30_az[ii])*degtorad,float(pos30_rng[ii])*1.852, marker='o', color='red')
    ax.plot(float(pos45_az[ii])*degtorad,float(pos45_rng[ii])*1.852, marker='o', color='orange')
    ax.plot(float(pos60_az[ii])*degtorad,float(pos60_rng[ii])*1.852, marker='o', color='purple')
    ii=ii+1

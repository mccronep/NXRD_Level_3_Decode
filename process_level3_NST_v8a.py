...
###ax.plot()
##ax.plot(0,0,'ro', label=str(RADAR_3_LETTER), markersize=19)
...
for item in storm_id:
    ...
    #ax.plot(xlo,ylo,'o', label=storm_id[ii], color='blue', transform=ccrs.Geodetic())
    ax.plot(lon2,lat2,'o', label=storm_id[ii], color='blue', transform=ccrs.Geodetic())
    lat2, lon2 = get_point_at_distance(cent_lat, cent_lon, float(pos15_rng[ii])*nm2km, float(pos15_az[ii]))
    ax.plot(lon2,lat2,'o', label=storm_id[ii], color='green', transform=ccrs.Geodetic())
    lat2, lon2 = get_point_at_distance(cent_lat, cent_lon, float(pos30_rng[ii])*nm2km, float(pos30_az[ii]))
    ax.plot(lon2,lat2,'o', label=storm_id[ii], color='red', transform=ccrs.Geodetic())

    lat2, lon2 = get_point_at_distance(cent_lat, cent_lon, float(pos45_rng[ii])*nm2km, float(pos45_az[ii]))
    ax.plot(lon2,lat2,'o', label=storm_id[ii], color='orange', transform=ccrs.Geodetic())

    lat2, lon2 = get_point_at_distance(cent_lat, cent_lon, float(pos60_rng[ii])*nm2km, float(pos60_az[ii]))
    ax.plot(lon2,lat2,'o', label=storm_id[ii], color='purple', transform=ccrs.Geodetic())
